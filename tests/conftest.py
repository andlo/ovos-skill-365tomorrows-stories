"""Shared pytest fixtures for the 365tomorrows-stories skill test suite."""
...
_spec = importlib.util.spec_from_file_location("tomorrows_skill", _INIT_PATH)
_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_module)

TomorrowsStories = _module.TomorrowsStories
...
def skill(tmp_path, monkeypatch):
    s = TomorrowsStories.__new__(TomorrowsStories)
    s.log = MagicMock()
    s.skill_id = "ovos-skill-365tomorrows-stories.test"
    s.status = MagicMock()
    s._bus = MagicMock()
    s._settings = {}
    monkeypatch.setattr(TomorrowsStories, "lang", "en-us", raising=False)
    ...
    s._collection_aliases = ["365 tomorrows", "three sixty five tomorrows", "tomorrows"]
    return s
